...
def test_validate_rejects_a_placement_referencing_a_deleted_node(page):
    result = page.evaluate("""() => {
      ...
      data.threats.push({ id: 'PLACEMENT_99', nodeId: 'T_999', x: 1, y: 1, w: 1, h: 1, pageId: data.pages[0].id });
      const fresh = Bowtie.BowtieModel.fromJSON(data);
      return Bowtie.DocumentSerializer.validate(fresh);
    }""")
    assert result["ok"] is False
    assert "T_999" in result["error"]
...
def test_load_from_json_throws_and_changes_nothing_on_a_broken_document(page):
    ...
    result = page.evaluate("""() => {
      ...
      broken.threats.push({ id: 'PLACEMENT_99', nodeId: 'T_999', x: 1, y: 1, w: 1, h: 1, pageId: broken.pages[0].id });
...
def test_a_corrupt_import_shows_a_message_and_leaves_the_canvas_untouched(page):
    ...
    data["consequences"].append({
        "id": "PLACEMENT_99", "nodeId": "C_999", "x": 1200, "y": 200, "w": 140, "h": 60,
        "pageId": data["pages"][0]["id"],
    })
    data["lines"].append({
        "id": "LINE_99", "originType": "consequence", "originId": "PLACEMENT_99",
    ...
    state = page.evaluate("""() => ({
      threatCount: window.__lastUndo.rawModel.threats.length,
      consequenceCount: window.__lastUndo.rawModel.consequences.length,
    })""")
