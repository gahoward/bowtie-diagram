...
strategy"). Driven via blank_page.evaluate against window.Bowtie.Decimal, the
same way every other JS unit in this app is tested.
"""


def test_parse_plain_decimal_round_trips_exactly(blank_page):
...
def test_parse_scientific_notation_round_trips(blank_page):
    result = blank_page.evaluate("""() => {
      return [
...
def test_parse_integers_and_negative_values(blank_page):
    result = blank_page.evaluate("""() => {
      return [
...
def test_parse_rejects_garbage(blank_page):
    threw = blank_page.evaluate("""() => {
...
def test_multiply_is_exact_across_a_long_chain(blank_page):
...
def test_multiply_combines_mantissa_and_exponent(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_add_is_exact_across_different_exponents(blank_page):
...
def test_add_same_exponent_and_negative_values(blank_page):
    result = blank_page.evaluate("""() => {
      return [
...
def test_compare_exact_at_a_boundary_value(blank_page):
...
def test_compare_different_exponents(blank_page):
    result = blank_page.evaluate("""() => {
      return {
...
def test_max_picks_the_largest_of_several_values(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_max_of_empty_list_is_null(blank_page):
    result = blank_page.evaluate("() => Bowtie.Decimal.max([])")
    assert result is None


def test_hour_to_year_and_back_round_trips_within_rounding_precision(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_year_to_hour_conversion_matches_direct_division(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_conversion_never_mutates_the_original_value(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_to_display_number_rounds_to_significant_figures(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_negate_and_is_zero(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_parse_and_round_trip_one_times_ten_to_the_minus_fifteen(blank_page):
    result = blank_page.evaluate("""() => {
      return [
...
def test_round_trip_extreme_magnitudes_both_directions(blank_page):
...
def test_compare_distinguishes_values_finer_than_double_precision(blank_page):
...
def test_compare_at_exactly_one_times_ten_to_the_minus_fifteen_boundary(blank_page):
...
def test_multiply_of_two_extremely_small_values_stays_exact(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_multiply_chain_of_ten_barrier_reductions_reaching_1e_minus_15_is_exact(blank_page):
...
def test_round_trip_seventeen_significant_digits_with_no_truncation(blank_page):
    ...
    result = blank_page.evaluate("(v) => Bowtie.Decimal.parse(v).toDecimalString()", value)
    assert result == '0.' + '0' * 14 + '12345678901234567'


def test_normalize_strips_trailing_zeros_without_changing_the_represented_value(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_to_display_number_stays_accurate_at_1e_minus_15_magnitude(blank_page):
    result = blank_page.evaluate("""() => {
...
def test_convert_hour_year_precise_at_an_extremely_small_frequency(blank_page):
...
def test_parse_handles_a_fifty_digit_mantissa_without_overflow(blank_page):
    # BigInt is arbitrary-precision -- there is no digit-count ceiling the
    # way there would be with a native Number.
    digits = "1" + "0" * 49
    result = blank_page.evaluate("(v) => Bowtie.Decimal.parse(v).toDecimalString()", digits)
    assert result == digits


def test_parse_accepts_a_leading_plus_sign(blank_page):
    result = blank_page.evaluate("() => Bowtie.Decimal.parse('+1.5E-15').toDecimalString()")
