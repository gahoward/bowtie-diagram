...
def test_first_barrier_lands_right_of_its_threat(page):
    page.evaluate("() => { window.__lastModel.addThreat({x: 150, y: 200}); }")
    page.wait_for_timeout(100)

    line_el = page.locator('[data-role="threat-direct"]').first
    ...
    state = page.evaluate("""() => {
      ...
      return { threatX: threat.x, pbX: pb.x };
    }""")
    assert state["pbX"] > state["threatX"]
...
def test_barrier_inserted_before_lands_left_of_its_anchor(page):
    ...
    # Click just past the threat's own edge (clear of PB_1's box) to land in
    # the gap between the threat and PB_1.
    line_el = page.locator('[data-role="threat-line"]').first
    box = line_el.bounding_box()
    ...
    state = page.evaluate("""() => {
      ...
      return { stops: line.stops, xs, threatX: m.threats[0].x };
    }""")
    assert state["xs"] == sorted(state["xs"]), "stops must stay in increasing-x order"
    assert all(x > state["threatX"] for x in state["xs"])
...
def test_shift_toward_tle_reorders_a_mitigative_barrier_the_mirrored_way(page):
    """Mirrors the Preventative case: MB_1 (nearest the Consequence) shifted
    ...
    page.evaluate("""() => {
      ...
      m.addMitigativeControl(m.consequences[0].id); // MB_1, nearest the consequence
      m.addMitigativeControl(m.consequences[0].id); // MB_2, appended further toward the TLE
    }""")
    page.wait_for_timeout(100)
    ...
    state = page.evaluate("""() => {
      const m = window.__lastModel;
      const line = m._lineFor(m.consequences[0].id);
...
def test_shift_toward_tle_on_a_shared_barrier_prompts_for_which_path(page):
    """Two Threats each have their own private first barrier, then both
    ...
    Threat's own private barrier), so it must prompt for which path(s) to
    reorder rather than guessing. Confirming with only T_1's path checked
    must reorder that line and leave T_2's line untouched."""
    ids = page.evaluate("""() => {
      const m = window.__lastModel;
    ...
    # The picker lists both origins ("T_1"/"T_2", per _openLineSelectModal's
    # {key: line.id, label: line.originId}); check only T_1's row.
    page.locator(".modal-checkbox-row", has_text="T_1").locator("input[type=checkbox]").check()
    ...
    state = page.evaluate("""() => {
      const m = window.__lastModel;
      return {
        line1: m._lineFor(m.threats[0].id).stops,
        line2: m._lineFor(m.threats[1].id).stops,
      };
    }""")
    assert state["line1"] == [ids["shared"], ids["pb1"]], "T_1's path must be reordered"
    assert state["line2"] == [ids["pb2"], ids["shared"]], "T_2's path, left unchecked, must be untouched"
...
def test_shift_shared_barrier_on_both_paths_does_not_corrupt_positions(page):
    ...
    right-click PB_3, "Shift Away From TLE", tick BOTH T_1 and T_2 (not
    ...
    state = page.evaluate("""() => {
      const m = window.__lastModel;
      return {
        c1: m._lineFor('T_1').stops,
        c2: m._lineFor('T_2').stops,
      };
    }""")
    pb1_id = placement_id_for_node(page, "PB_1")
    pb2_id = placement_id_for_node(page, "PB_2")
...
def test_shift_shared_barrier_on_both_paths_then_auto_arrange_has_no_overlap(page):
    ...
    to leave PB_1's label overlapping PB_2's box, because T_1/T_2 (now
