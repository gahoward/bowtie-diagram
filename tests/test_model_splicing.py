...
convention for Consequence lines (stops stored nearest-Consequence-first, opposite
of Threat lines) makes direction/sign mistakes easy and quiet — several bugs
...
def test_insert_after_places_new_barrier_toward_tle(page):
    result = page.evaluate("""() => {
      ...
      const pb1 = m.addPreventativeControl(threat.id);
      const pb2 = m.insertBarrier('preventativeBarrier', 'after', pb1.id);
      return { stops: m._lineFor(threat.id).stops, pb1x: pb1.x, pb2x: pb2.x, pb1: pb1.id, pb2: pb2.id };
    }""")
...
def test_mitigative_after_before_mirror_preventative(page):
    ...
    from the origin", but for an Consequence that's toward the consequence, not
    toward the TLE."""
    result = page.evaluate("""() => {
      const m = window.__lastModel;
      m.addConsequence({x: 1200, y: 200});
      const consequence = m.consequences[0];
      const mb1 = m.addMitigativeControl(consequence.id);
      ...
      return {
        stops: m._lineFor(consequence.id).stops,
        mb1x: mb1.x, afterX: mbAfter.x, beforeX: mbBefore.x,
      };
    }""")
    assert result["afterX"] > result["mb1x"], "MB 'after' moves toward the Consequence (larger x)"
...
def test_attach_input_defaults_to_inheriting_downstream(page):
    ...
    Threat to a barrier that already continues further (for another Threat)
    ...
    result = page.evaluate("""() => {
      ...
      const pb2 = m.insertBarrier('preventativeBarrier', 'after', pb1.id); // T_1: [PB_1, PB_2]
      m.addThreat({x: 150, y: 300});
      m.attachInputToPreventativeControl(m.threats[1].id, pb1.id); // no third arg
      return { stops: m._lineFor(m.threats[1].id).stops, pb1: pb1.id, pb2: pb2.id };
    }""")
    assert result["stops"] == [result["pb1"], result["pb2"]]


def test_attach_input_declining_inherit_stops_at_the_target_barrier(page):
    """Reported bug: attaching a bare Threat to a barrier that already
    continues further for another Threat must NOT be forced to also adopt
    ...
    result = page.evaluate("""() => {
      ...
      m.addThreat({x: 150, y: 300});
      m.attachInputToPreventativeControl(m.threats[1].id, pb1.id, false);
      return { stops: m._lineFor(m.threats[1].id).stops, pb1: pb1.id };
    }""")
...
def test_attach_input_declining_inherit_keeps_the_lines_own_prior_chain(page):
    """A Threat that already had its own further barriers before this
    attach keeps THOSE when declining to inherit the target's -- not just
    a bare dead-end at the target. Re-pointing T_2's existing chain
    (currently through its own barrier) at T_1's first barrier (which
    continues further, for T_1) while declining that continuation must
    keep T_2's own further barrier, not adopt T_1's."""
    result = page.evaluate("""() => {
      const m = window.__lastModel;
      m.addThreat({x: 150, y: 90});
      const pb1 = m.addPreventativeControl(m.threats[0].id);
      const pb1Continuation = m.insertBarrier('preventativeBarrier', 'after', pb1.id); // T_1: [pb1, pb1Continuation]
      m.addThreat({x: 150, y: 300});
      const ownPb = m.addPreventativeControl(m.threats[1].id); // T_2: [ownPb]
      m.attachInputToPreventativeControl(m.threats[1].id, pb1.id, false);
      return { stops: m._lineFor(m.threats[1].id).stops, pb1: pb1.id, ownPb: ownPb.id, pb1Continuation: pb1Continuation.id };
    }""")
...
def test_attach_output_inherits_downstream_by_default(page):
    result = page.evaluate("""() => {
      ...
      const mb2 = m.addMitigativeControl(m.consequences[0].id); // C_1: [mb1, mb2]
      m.addConsequence({x: 1200, y: 300});
      m.attachOutputToMitigativeControl(mb1.id, m.consequences[1].id);
      return { stops: m._lineFor(m.consequences[1].id).stops, mb1: mb1.id, mb2: mb2.id };
    }""")
    assert result["stops"] == [result["mb1"], result["mb2"]]


def test_attach_output_declining_inherit_stops_at_the_target_barrier(page):
    """Mirrors the Threat-side declined case for Consequences/MBs."""
    result = page.evaluate("""() => {
      const m = window.__lastModel;
      m.addConsequence({x: 1200, y: 90});
      const mb1 = m.addMitigativeControl(m.consequences[0].id);
      m.addMitigativeControl(m.consequences[0].id); // C_1: [mb1, mb2]
      m.addConsequence({x: 1200, y: 300});
      m.attachOutputToMitigativeControl(mb1.id, m.consequences[1].id, false);
      return { stops: m._lineFor(m.consequences[1].id).stops, mb1: mb1.id };
    }""")
...
def test_donor_continuation_is_empty_when_the_target_has_no_further_barrier(page):
    ...
    result = page.evaluate("""() => {
      const m = window.__lastModel;
      m.addThreat({x: 150, y: 90});
      const pb1 = m.addPreventativeControl(m.threats[0].id); // T_1: [PB_1], nothing further
...
def test_connect_directly_to_tle_consequence_side_keep_through_barrier(page):
    """Consequence lines store stops nearest-Consequence-first (the opposite of
    Threat lines): repeated addMitigativeControl calls always append at the
    TLE-facing tail, so MB_1/MB_2/MB_3 end up nearest-consequence to
    ...
    result = page.evaluate("""() => {
      const m = window.__lastModel;
      m.addConsequence({x: 1200, y: 200});
      const o = m.consequences[0];
      const mb1 = m.addMitigativeControl(o.id); // nearest consequence
...
def test_swap_barrier_with_neighbor_is_a_noop_at_the_end_of_the_chain(page):
    ...
    result = page.evaluate("""() => {
      ...
      const line = m._lineFor(m.threats[0].id);
...
def test_swap_barrier_with_neighbor_only_touches_the_named_line(page):
    """Two Threats sharing a downstream barrier each have their own private
    first barrier. Reordering the shared barrier within T_1's own line
    must not perturb T_2's line, even though both lines pass through it."""
    result = page.evaluate("""() => {
      const m = window.__lastModel;
      m.addThreat({x: 150, y: 90});
      const pb1 = m.addPreventativeControl(m.threats[0].id);
      const shared = m.insertBarrier('preventativeBarrier', 'after', pb1.id); // T_1: [PB_1, shared]
      m.addThreat({x: 150, y: 300});
      const pb2 = m.addPreventativeControl(m.threats[1].id);
      m.attachExistingBarrier('preventativeBarrier', 'after', pb2.id, shared.id); // T_2: [PB_2, shared]
      const line1 = m._lineFor(m.threats[0].id);
      const line2 = m._lineFor(m.threats[1].id);
      const before2 = line2.stops.slice();
      m.swapBarrierWithNeighbor(line1.id, shared.id, false); // shift shared away from the TLE, within T_1's line only
      return { stops1: line1.stops, stops2: line2.stops, before2, pb1: pb1.id, pb2: pb2.id, shared: shared.id };
    }""")
    assert result["stops1"] == [result["shared"], result["pb1"]], "T_1's line must be reordered"
...
def test_swap_barrier_with_neighbor_is_undoable(page):
    ...
    result = page.evaluate("""() => {
      ...
      const pb1 = m.addPreventativeControl(threat.id);
      const pb2 = m.insertBarrier('preventativeBarrier', 'after', pb1.id);
      const before = window.__lastModel._lineFor(threat.id).stops.slice();
      const lineId = window.__lastModel._lineFor(threat.id).id;
      m.swapBarrierWithNeighbor(lineId, pb1.id, true);
      const swapped = window.__lastModel._lineFor(threat.id).stops.slice();
      undo.undo();
      const restored = window.__lastModel._lineFor(threat.id).stops.slice();
...
def test_swap_barrier_with_neighbor_batch_still_swaps_x_when_all_lines_agree(page):
    ...
    result = page.evaluate("""() => {
      ...
      const pb2 = m.insertBarrier('preventativeBarrier', 'after', pb1.id); // T_1: [PB_1, PB_2]
      m.addThreat({x: 150, y: 300});
      m.attachInputToPreventativeControl(m.threats[1].id, pb1.id); // T_2 also: [PB_1, PB_2]
      const line1 = m._lineFor(m.threats[0].id);
      const line2 = m._lineFor(m.threats[1].id);
...
def test_swap_barrier_with_neighbor_batch_is_one_undo_step(page):
    result = page.evaluate("""() => {
      ...
      const before = {
        stops1: window.__lastModel._lineFor(window.__lastModel.threats[0].id).stops.slice(),
        stops2: window.__lastModel._lineFor(window.__lastModel.threats[1].id).stops.slice(),
      };
      const line1Id = window.__lastModel._lineFor(window.__lastModel.threats[0].id).id;
      const line2Id = window.__lastModel._lineFor(window.__lastModel.threats[1].id).id;
