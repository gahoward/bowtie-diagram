...
def _page_undo_depth(page):
    ...
    here. A bare addThreat/addConsequence/addPreventativeControl/
...
def test_undo_then_redo_round_trips_through_an_action(page):
    page.evaluate("() => { window.__lastUndo.model.addThreat({x: 150, y: 200}); }")
...
def test_new_action_after_undo_discards_redo_history(page):
    ...
    page.evaluate("() => window.__lastUndo.undo();")  # back to 1 threat, redo has the 2nd
    assert _undo_redo_disabled(page)["redo"] is False

    page.evaluate("() => { window.__lastUndo.model.addConsequence({x: 1200, y: 200}); }")
    assert _undo_redo_disabled(page)["redo"] is True  # discarded, not just unused

    state = page.evaluate("() => ({ threats: window.__lastModel.threats.length, consequences: window.__lastModel.consequences.length })")
    assert state == {"threats": 1, "consequences": 1}
...
def test_stack_is_capped_at_fifty(page):
    # Each bare addThreat() creates a brand-new node -- a document-tier step
    # (node_library_proposal.md "Undo tier") -- so the cap being tested here
    # is the document stack's, not the page stack's.
    page.evaluate("""() => {
      const m = window.__lastUndo.model;
      for (let i = 0; i < 55; i += 1) m.addThreat({ x: 150, y: 100 + i });
    }""")
    assert _document_undo_depth(page)["undo"] == 50
    assert page.evaluate("() => window.__lastModel.threats.length") == 55

    page.evaluate("""() => {
      for (let i = 0; i < 50; i += 1) window.__lastUndo.undo();
    }""")
    # Only the last 50 additions are undoable; the first 5 Threats remain.
    assert page.evaluate("() => window.__lastModel.threats.length") == 5
...
def test_drag_gesture_is_one_undo_step_not_one_per_move(page):
    ...
    node = page.locator('.node.threat').first
    ...
    assert moved_y != 200
    page.evaluate("() => window.__lastUndo.undo();")
    assert page.evaluate("() => window.__lastModel.threats[0].y") == 200
...
def test_clicking_a_node_to_inspect_it_does_not_touch_undo_redo_history(page):
    ...
    Threat/Consequence to dim unrelated elements" (explicitly non-mutating)
    ...
    # same position, no movement) on a Threat — purely inspecting it via
