"""Auto-arrange's layout guarantees: the topmost Threat/Consequence must clear
...
# Threats/PreventativeBarriers and Consequences/MitigativeBarriers, mixing three
...
_BUILD_RANDOM_TOPOLOGY_JS = """(seed) => {
  ...
  buildSide(
    (opts) => m.addThreat(opts),
    (threatId) => m.addPreventativeControl(threatId),
    (pcId, threatId, inherit) => m.attachInputToPreventativeControl(threatId, pcId, inherit),
    150,
  );
  buildSide(
    (opts) => m.addConsequence(opts),
    (consequenceId) => m.addMitigativeControl(consequenceId),
    (mcId, consequenceId, inherit) => m.attachOutputToMitigativeControl(mcId, consequenceId, inherit),
    1200,
  );
}"""
...
_CHECK_NO_FOREIGN_ROW_IN_ANY_BOX_JS = """() => {
  ...
  const threats = m.threatsForPage(pageId).map((c) => ({ id: c.id, y: c.y }));
  const consequences = m.consequencesForPage(pageId).map((o) => ({ id: o.id, y: o.y }));
  ...
  m.preventativeBarriersForPage(pageId).forEach((pb) => check(pb, threats));
  m.mitigativeBarriersForPage(pageId).forEach((mb) => check(mb, consequences));
  return violations;
}"""
...
def test_random_topologies_keep_every_barriers_box_scoped_to_its_own_rows(page):
    ...
    random Threat/Consequence + barrier-sharing topologies at the same
...
def test_topmost_threat_and_consequence_clear_the_tle(page):
    page.evaluate("""() => {
      const m = window.__lastModel;
    ...
    positions = page.evaluate("""() => {
      const m = window.__lastModel;
      return { tleY: m.topLevelEvent.y, threatY: m.threats[0].y, consequenceY: m.consequences[0].y };
    }""")
    assert positions["threatY"] < positions["tleY"]
    assert positions["consequenceY"] < positions["tleY"]
    assert positions["tleY"] - positions["threatY"] >= 150
    assert positions["tleY"] - positions["consequenceY"] >= 150


def test_unrelated_barriers_never_overlap_vertically_after_arrange(page):
    page.evaluate("""() => {
      const m = window.__lastModel;
      // A merge group: two threats sharing one PB.
      m.addThreat({x: 150, y: 90});
      const a = m.threats[0];
      ...
      // An unrelated third threat with its own separate PB.
      m.addThreat({x: 150, y: 600});
      m.addPreventativeControl(m.threats[2].id);
    }""")
    auto_arrange(page)
    page.wait_for_timeout(150)
...
def test_merged_barriers_label_does_not_overlap_an_unrelated_barriers_box(page):
    ...
    Reproduces a real screenshot: two Threats merged into one PB, plus an
    unrelated third Threat's own separate PB positioned right after it."""
    page.evaluate("""() => {
      const m = window.__lastModel;
      m.addThreat({x: 150, y: 90});
      const a = m.threats[0];
      const pb = m.addPreventativeControl(a.id);
      m.addThreat({x: 150, y: 300});
      m.attachInputToPreventativeControl(m.threats[1].id, pb.id);
      m.addThreat({x: 150, y: 600});
      m.addPreventativeControl(m.threats[2].id);
    }""")
    auto_arrange(page)
...
def test_two_unrelated_lone_barriers_labels_and_boxes_never_overlap(page):
    ...
    unrelated Threats each with their own single-lane barrier (no sharing at
...
def test_threats_that_only_merge_two_hops_downstream_keep_their_own_first_barriers_clear(page):
    """Real screenshot: T_1 -> PB_A -> PB_C -> TLE, T_2 -> PB_B -> PB_C -> TLE.
    T_1 and T_2 only share a barrier at PB_C (two hops downstream of their
    ...
    page.evaluate("""() => {
      const m = window.__lastModel;
      m.addThreat({x: 150, y: 90}); // T_1
      const pbA = m.addPreventativeControl(m.threats[0].id);
      m.addThreat({x: 150, y: 300}); // T_2
      const pbB = m.addPreventativeControl(m.threats[1].id);
...
def test_sharing_two_hops_downstream_still_clusters_threats_together(page):
    """Two Threats can share a barrier without sharing their immediate next
    barrier: T_1 -> PB_1 -> PB_3 -> TLE, T_3 -> PB_4 -> PB_3 -> TLE — they
    only converge at PB_3, one hop after their own distinct first barrier.
    An unrelated T_2 (direct to the TLE) must not be laid out between them,
    or PB_3's grown box (spanning T_1's and T_3's lanes) visually
    intercepts T_2's own unrelated line."""
    page.evaluate("""() => {
      const m = window.__lastModel;
      m.addThreat({x: 150, y: 200}); // T_1
      const pb1 = m.addPreventativeControl(m.threats[0].id);
      const pb3 = m.insertBarrier('preventativeBarrier', 'after', pb1.id);

      m.addThreat({x: 150, y: 400}); // T_2, unrelated, direct to the TLE

      m.addThreat({x: 150, y: 600}); // T_3
      const pb4 = m.addPreventativeControl(m.threats[2].id);
    ...
    result = page.evaluate("""() => {
      ...
      return {
        threatYs: Object.fromEntries(m.threats.map((c) => [c.nodeId, c.y])),
        pb3Top: bounds.cy - bounds.h / 2,
        pb3Bottom: bounds.cy + bounds.h / 2,
      };
    }""")
    c2y = result["threatYs"]["T_2"]
    assert not (result["pb3Top"] < c2y < result["pb3Bottom"]), (
        "T_2's unrelated line must not pass through PB_3's grown box"
    )


def test_sharing_two_hops_downstream_still_clusters_consequences_together(page):
    """Mirrors the Threat-side case above for Consequences/MBs."""
    page.evaluate("""() => {
      const m = window.__lastModel;
      m.addConsequence({x: 1200, y: 200});
      const mb1 = m.addMitigativeControl(m.consequences[0].id);
      const mb3 = m.insertBarrier('mitigativeBarrier', 'after', mb1.id);

      m.addConsequence({x: 1200, y: 400}); // unrelated, direct to the TLE

      m.addConsequence({x: 1200, y: 600});
      const mb4 = m.addMitigativeControl(m.consequences[2].id);
    ...
    result = page.evaluate("""() => {
      ...
      return {
        consequenceYs: Object.fromEntries(m.consequences.map((o) => [o.nodeId, o.y])),
        mb3Top: bounds.cy - bounds.h / 2,
        mb3Bottom: bounds.cy + bounds.h / 2,
      };
    }""")
    o2y = result["consequenceYs"]["C_2"]
    assert not (result["mb3Top"] < o2y < result["mb3Bottom"]), (
        "C_2's unrelated line must not pass through MB_3's grown box"
    )


def test_two_bridged_merge_columns_keep_each_others_unrelated_threat_clear(page):
    """A Threat can bridge two otherwise-unconnected merge groups: T_1 shares
    PB_1 with T_3, and (separately) T_2 shares PB_2 with T_3. Grouping by a
    single shared key isn't enough here — T_1 and T_2 don't share anything
    directly, only via T_3 — so this needs the two groups to actually be
    ordered (not just clustered) as T_1, T_3, T_2 (or its mirror image), or
    whichever threat ends up in the middle has an unrelated barrier's grown
    box cutting through its own line."""
    page.evaluate("""() => {
      const m = window.__lastModel;
      m.addThreat({x: 150, y: 200}); // T_1
      m.addPreventativeControl(m.threats[0].id); // PB_1

      m.addThreat({x: 150, y: 400}); // T_2
      m.addPreventativeControl(m.threats[1].id); // PB_2

      m.addThreat({x: 150, y: 600}); // T_3 - bridges both
      const c3 = m.threats[2];
      m.attachInputToPreventativeControl(c3.id, 'PB_1'); // T_3: [PB_1]
    ...
    c1y, c2y = result["threatYs"]["T_1"], result["threatYs"]["T_2"]

    assert not (pb1_top < c2y < pb1_bot), "T_2 (no connection to PB_1) must not fall inside PB_1's grown span"
    assert not (pb2_top < c1y < pb2_bot), "T_1 (no connection to PB_2) must not fall inside PB_2's grown span"


def test_two_bridged_merge_columns_consequence_side(page):
    """Mirrors the bridged-Threat case above for Consequences/MBs."""
    page.evaluate("""() => {
      const m = window.__lastModel;
      m.addConsequence({x: 1200, y: 200}); // C_1
      m.addMitigativeControl(m.consequences[0].id); // MB_1

      m.addConsequence({x: 1200, y: 400}); // C_2
      m.addMitigativeControl(m.consequences[1].id); // MB_2

      m.addConsequence({x: 1200, y: 600}); // C_3 - bridges both
      const o3 = m.consequences[2];
      m.attachOutputToMitigativeControl('MB_1', o3.id); // C_3: [MB_1]
    ...
    o1y, o2y = result["consequenceYs"]["C_1"], result["consequenceYs"]["C_2"]

    assert not (mb1_top < o2y < mb1_bot), "C_2 (no connection to MB_1) must not fall inside MB_1's grown span"
    assert not (mb2_top < o1y < mb2_bot), "C_1 (no connection to MB_2) must not fall inside MB_2's grown span"


def test_auto_arrange_leaves_room_for_a_long_wrapped_threat_name(page):
    """architecture review finding: ROW_SPACING/GROUP_GAP were derived only
    from fixed barrier geometry and never referenced a Threat/Consequence's OWN
    ...
    Layout.threatConsequenceBounds wraps a long name to more lines. Two Threats
    ...
    rendered height (the same Layout.threatConsequenceBounds call the live
    render uses) and only ever grows the gap to fit, never shrinks it."""
    long_name = (
        "This threat has a long, realistic multi-sentence description of exactly the kind a real "
    ...
    page.evaluate(
        """(longName) => {
          ...
          const pb = m.addPreventativeControl(m.threats[0].id);
          m.addThreat({x: 150, y: 400});
          m.attachInputToPreventativeControl(m.threats[1].id, pb.id);
          m.renameElement(m.threats[1].id, longName);
        }""",
        long_name,
    )
    auto_arrange(page)
    ...
    result = page.evaluate("""() => {
      const m = window.__lastModel;
      const view = window.__lastView;
      return m.threats.map((c) => ({ y: c.y, h: view.boundsById[c.id].h }));
    }""")
    a, b = result
    a_top, a_bot = a["y"] - a["h"] / 2, a["y"] + a["h"] / 2
    b_top, b_bot = b["y"] - b["h"] / 2, b["y"] + b["h"] / 2
    assert not (a_top < b_bot and b_top < a_bot), "the long-named Threat's own box must not overlap its sibling's"


def test_lone_barrier_lands_in_the_tle_adjacent_column_not_the_threat_adjacent_one(page):
    """Regression test: a Threat with its own single, unchained Preventative
    Barrier (nothing further before the TLE) must land in the SAME column as
    another chain's TLE-adjacent barrier, not the column nearest the Threats
    -- even though, counted from ITS OWN Threat, it's only one hop deep. Depth
    ...
    a Preventative Barrier to a bare Threat (like T_4) landed it next to
    PB_1/PB_2 instead of alongside PB_3, the column both of those chains
    that instead feed the TLE, real screenshot."""
    page.evaluate("""() => {
      const m = window.__lastModel;
      // A two-deep chain: two Threats each with their own first barrier,
      ...
      // A separate, unrelated Threat with a single unchained barrier of its
    ...
    assert positions["loneX"] != positions["pbAX"], (
        "the lone barrier's column must differ from the threat-adjacent "
...
def test_shared_barrier_with_a_divergent_downstream_chain_gets_its_own_column(page):
    ...
    T_2 to PB_1 by inserting it in front of T_2's own existing PB_2 --
    i.e. "Attach to Existing Preventative Barrier" on the T_2-PB_2 line
    segment, picking PB_1, not the Threat-node menu's replace-the-chain
    version):

      T_1 -> PB_1 -> PB_3        (PB_1's line to T_1 ends at PB_3)
      T_2 -> PB_1 -> PB_2 -> PB_3 (PB_1's line to T_2 continues through PB_2)
    ...
    `model.lines` (T_1's, here) -- giving PB_1 depth 2 (1 + PB_3's depth of
    ...
    PB_1's only leafward neighbours are T_1 and T_2 while PB_2's only
    ...
    page.evaluate("""() => {
      ...
      const c2Line = m.lines.find((l) => l.originId === m.threats[1].id);
    ...
    assert positions["pb1"]["x"] < positions["middle"]["x"] < positions["pb3"]["x"], (
        ...
        "in that order from the threats side toward the TLE"
    )
...
def test_two_mutually_bare_threats_use_row_spacing_not_group_gap(page):
    """Reported bug: with several Threats/Consequences and few barriers, the
    ...
    Threats that have no barrier at all between them to protect. Two fully
    bare Threats next to each other in the order have nothing barrier-shaped
    to collide, so they should get the same compact ROW_SPACING two
    barrier-sharing siblings get, not the full worst-case gap."""
    page.evaluate("""() => {
      const m = window.__lastModel;
      m.addThreat({x: 150, y: 90});
      m.addThreat({x: 150, y: 90}); // both bare -- _findClearY nudges this one down
    }""")
    auto_arrange(page)
    page.wait_for_timeout(150)

    ys = page.evaluate("() => window.__lastModel.threats.map((c) => c.y).sort((a, b) => a - b)")
    assert ys[1] - ys[0] == 110, "two mutually bare Threats must be exactly ROW_SPACING apart"


def test_a_bare_threat_next_to_a_barrier_bearing_one_still_gets_full_group_gap(page):
    """Regression guard for the fix above: the compaction must be scoped
    strictly to BOTH sides being bare. A bare Threat sitting next to one
    ...
    page.evaluate("""() => {
      ...
      m.addThreat({x: 150, y: 90});
      m.addPreventativeControl(m.threats[0].id);
      m.addThreat({x: 150, y: 90}); // bare, nudged below T_1 by _findClearY
    }""")
    auto_arrange(page)
    page.wait_for_timeout(150)

    ys = page.evaluate("() => window.__lastModel.threats.map((c) => c.y).sort((a, b) => a - b)")
    assert ys[1] - ys[0] == 220, "a bare Threat beside a barrier-bearing one must keep the full GROUP_GAP"


def test_mixed_bare_and_barrier_rows_never_overlap_in_either_spacing_mode(page):
    """Stress check for the compaction fix: five Threats with only two
    ...
    def build(page):
        page.evaluate("""() => {
          const m = window.__lastModel;
          for (let i = 0; i < 5; i += 1) m.addThreat({x: 150, y: 90 + i * 50});
          m.addPreventativeControl(m.threats[0].id);
          m.addPreventativeControl(m.threats[2].id);
        }""")
    ...
    def violations(page):
        return page.evaluate("""() => {
          ...
          // barrier's own x IS its model x; a Threat/Consequence's bounds don't
          ...
          const m = window.__lastModel;
          const allIds = [
            ...m.threats, ...m.consequences, ...m.preventativeBarriers, ...m.mitigativeBarriers,
          ].map((n) => n.id);
...
def test_diverging_after_a_shared_barrier_keeps_full_clearance_between_rows(page):
    """Reported bug: two Threats merge into one shared barrier, then each
    ...
    shape). assignLeafYs used to grant these two Threats only ROW_SPACING
    ...
    page.evaluate("""() => {
      ...
      m.addThreat({x: 150, y: 300});
      const ownB = m.addPreventativeControl(m.threats[1].id);
      m.attachExistingBarrier('preventativeBarrier', 'after', ownB.id, shared.id); // T_2: [ownB, shared]
      // Now reorder `shared` away from the TLE on BOTH lines, exactly like
      // ticking both Threats in the "Shift Away From TLE" picker:
      const line1 = m._lineFor(m.threats[0].id).id;
      const line2 = m._lineFor(m.threats[1].id).id;
      m.swapBarrierWithNeighbor([line1, line2], shared.id, false);
      // Lines are now T_1: [shared, ownA], T_2: [shared, ownB] -- shared,
...
def test_a_tighter_shared_barrier_clusters_correctly_within_a_broader_shared_group(page):
    ...
    demo (T_1 -> PB_1 -> PB_3, T_2 -> PB_2 -> PB_3, T_3 -> PB_3 directly,
    T_4 bare), then attach T_4 to PB_2. T_2 and T_4 now privately share
    PB_2 (just the two of them), while T_1, T_2, T_3, and T_4 ALL
    ...
    region" (PB_3, four participants) -- T_3 (index 2) got visited before
    T_4 (index 3) purely because of array order, landing between T_2 and
    T_4 and making PB_2's grown box balloon to cover T_3's own row even
    though T_3 was never attached to PB_2 at all. The tighter (fewer-
    participant) relationship must now win: T_2 and T_4 end up strictly
    adjacent, and PB_2's box must not reach T_3's row."""
    page.evaluate("""() => {
      window.__lastModel.loadFromJSON(Bowtie.DEMO_DATA);
      window.__lastModel.attachInputToPreventativeControl('T_4', 'PB_2');
    }""")
    auto_arrange(page)
    page.wait_for_timeout(150)
    ...
    result = page.evaluate("""() => {
      const m = window.__lastModel;
      const view = window.__lastView;
      const threatYs = Object.fromEntries(m.threats.map((c) => [c.nodeId, c.y]));
      const b = view.boundsById['PB_2'];
      return { threatYs, pb2Top: b.cy - b.h / 2, pb2Bottom: b.cy + b.h / 2 };
    }""")
    c3y = result["threatYs"]["T_3"]
    assert not (result["pb2Top"] < c3y < result["pb2Bottom"]), (
        "T_3's row must not fall inside PB_2's grown box -- T_3 was never attached to PB_2"
    )
    c2y, c4y = result["threatYs"]["T_2"], result["threatYs"]["T_4"]
    other_ys = [y for cid, y in result["threatYs"].items() if cid not in ("T_2", "T_4")]
    assert not any(min(c2y, c4y) < y < max(c2y, c4y) for y in other_ys), (
        "T_2 and T_4 (sharing PB_2 privately) must end up strictly adjacent, "
        "with no other Threat's row landing between them"
    )
...
def test_a_lone_barrier_shared_only_by_one_member_of_a_broader_group_stays_out_of_its_box(page):
    ...
    T_4 declines to inherit PB_2's continuation to PB_3 (chooses "Stop
    Here"), so T_4 ends up sharing ONLY PB_2 with T_2 -- unlike the test
    above, T_4 is NOT a member of PB_3's shared group at all here. The
    first fix for this bug (buildConsecutiveOrder) only merged hyperedges
    at the top level, so once PB_3's 3-way group (T_1, T_2, T_3) had
    already been built into one block, a smaller hyperedge fully NESTED
    inside that block (PB_2: just T_2 and T_4) was never revisited -- T_4
    ...
    page.evaluate("""() => {
      ...
      m.attachInputToPreventativeControl('T_4', 'PB_2', false); // "Stop Here"
    }""")
    auto_arrange(page)
    page.wait_for_timeout(150)
    ...
    result = page.evaluate("""() => {
      ...
      // barrier's box on one page has no relationship to a Threat's row on
      // another page (they aren't even rendered together).
      const pageThreats = m.threats.filter((c) => c.pageId === 'PAGE_1');
      const pagePbs = m.preventativeBarriers.filter((pb) => pb.pageId === 'PAGE_1');
      const threatYs = Object.fromEntries(pageThreats.map((c) => [c.id, c.y]));
      ...
      return { threatYs, throughIds: Object.fromEntries(
    ...
    for pb_id, bounds in result["boundsById"].items():
        ...
        for threat_id, y in result["threatYs"].items():
            if threat_id in through:
                continue
            assert not (top < y < bottom), (
                f"{threat_id}'s row must not fall inside {pb_id}'s grown box -- "
                f"{threat_id}'s line never passes through {pb_id}"
            )
...
def test_a_chain_of_overlapping_pairs_keeps_every_barriers_box_disjoint(page):
    ...
    C_1 and C_3 share MB_1, then C_1 continues to MB_2 (also shared with
    C_2) while C_3 continues to MB_3 (also shared with C_4). That's three
    overlapping pairs chained together: {C_1,C_3} via MB_1, {C_1,C_2} via
    MB_2, {C_3,C_4} via MB_3. buildConsecutiveOrder's promoteToEdge used to
    ...
    merging MB_3's pair repositioned MB_1's block next to C_4 without
    pushing C_3 (not C_1) to the boundary touching it, leaving the row
    order C_2, C_3, C_1, C_4 instead of a valid C_2, C_1, C_3, C_4 (or its
    mirror). That breaks BOTH MB_2 (C_1/C_2, no longer contiguous) and
    MB_3 (C_3/C_4) at once, and their grown boxes end up overlapping each
    other despite sharing no consequence at all."""
    page.evaluate("""() => {
      const m = window.__lastModel;
      m.addConsequence({x: 1200, y: 200}); // C_1
      m.addMitigativeControl(m.consequences[0].id); // MB_1 -- C_1: [MB_1]
      m.addMitigativeControl(m.consequences[0].id); // MB_2 -- C_1: [MB_1, MB_2]

      m.addConsequence({x: 1200, y: 400}); // C_2
      m.attachOutputToMitigativeControl('MB_2', m.consequences[1].id); // C_2: [MB_2]

      m.addConsequence({x: 1200, y: 600}); // C_3
      m.attachOutputToMitigativeControl('MB_1', m.consequences[2].id, false); // "Stop Here" -- C_3: [MB_1]
      m.addMitigativeControl(m.consequences[2].id); // MB_3 -- C_3: [MB_1, MB_3]

      m.addConsequence({x: 1200, y: 800}); // C_4
      m.attachOutputToMitigativeControl('MB_3', m.consequences[3].id); // C_4: [MB_3]
    }""")
    auto_arrange(page)
    page.wait_for_timeout(150)
    ...
    assert mb2_bot <= mb3_top or mb3_bot <= mb2_top, (
        f"MB_2 (C_1/C_2) and MB_3 (C_3/C_4) share no consequence and must not overlap: "
